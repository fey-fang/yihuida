...
<!--right start-->
<div class="right">
<div class="Position"><span>你的位置：<a href="index.asp" >首页</a> > <a href="axw_list.asp"  >新闻资讯</a> > <a href="axw_list.asp?xwlb_id=<%=xwlb_id%>"  ><%=rs1("xwlb")%></a></span></div>
<div class="HeightTab clearfix"></div>
<!--main start-->
<div class="main">
...
上一篇：没有了
    <%else%>
上一篇：<a href='axw_view.asp?id=<%=rs11("id")%>' title='<%=rs11("xwbt")%>'><%=rs11("xwbt")%></a>&nbsp;&nbsp;<span class='ListDate'><%=formatdate(rs11("tjsj"),1)%></span>
<%end if%> </li>
...
下一篇：没有了
<%else%>
下一篇：<a href='axw_view.asp?id=<%=rs2("id")%>' title='<%=rs2("xwbt")%>'><%=rs2("xwbt")%></a>  <span class='ListDate'><%=formatdate(rs2("tjsj"),1)%></span>
<%end if%></li></ul>
</div>
